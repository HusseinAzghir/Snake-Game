...
public class GameDrawer extends JPanel implements KeyListener, ActionListener{
    ...
    public void paint(Graphics g)
    {
        ...
        g.drawString("Entwickelt von Hussein Azghir", 530, 60);
 
        FontMetrics fm = g.getFontMetrics();
 
        g.setFont(new Font("Arial", Font.PLAIN, 18));
 
        g.drawString("Bestes Ergebnis", 576, 110);
        g.drawRect(550, 120, 140, 30);
        g.drawString(bestScore+"", 550+(142-fm.stringWidth(bestScore+""))/2, 142);
 
        g.drawString("Gesamtpunktzahl", 573, 190);
        g.drawRect(550, 200, 140, 30);
        g.drawString(totalScore+"", 550+(142-fm.stringWidth(totalScore+""))/2, 222);
 
        g.drawString("Obst gegessen", 575, 270);
        g.drawRect(550, 280, 140, 30);
        g.drawString(fruitEaten+"", 550+(142-fm.stringWidth(fruitEaten+""))/2, 302);
 
        g.setFont(new Font("Arial", Font.BOLD, 16));
        g.drawString("Kontrollen", 550, 360);
 
        g.setFont(new Font("Arial", Font.PLAIN, 14));
        g.drawString("Pause/Start: Leertaste", 550, 385);
        g.drawString("LookTo Up: Pfeil nach oben", 550, 410);
        g.drawString("LookTo Down: Pfeil nach unten", 550, 435);
        g.drawString("lookToLeft : Pfeil nach links", 550, 460);
        g.drawString("LookTo Right: Pfeil nach rechts", 550, 485);
        ...
        for(int i=1; i<lengthOfSnake; i++)
        {
            if(boardX[i] == boardX[0] && boardY[i] == boardY[0])
            {
                ...
                g.drawString("Drücken Sie die Leertaste, um neu zu starten", 130, 260);
